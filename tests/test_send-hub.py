...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/send-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/send-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Send Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Send Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/send-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Transfers Page ---", flush=True)
    page.goto(f"{BASE}/send-hub/private/transfers", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_transfers.png"), full_page=True)
    ...
    page.goto(f"{BASE}/send-hub/private/queue", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_queue.png"), full_page=True)
    ...
    page.goto(f"{BASE}/send-hub/private/policies", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
