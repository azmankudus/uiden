...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/ticket-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/ticket-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Ticket Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Ticket Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/ticket-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Tickets Page ---", flush=True)
    page.goto(f"{BASE}/ticket-hub/private/tickets", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_tickets.png"), full_page=True)
    ...
    page.goto(f"{BASE}/ticket-hub/private/queue", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    ...
    page.goto(f"{BASE}/ticket-hub/private/reports", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_reports.png"), full_page=True)
