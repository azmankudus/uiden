...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/asset-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/asset-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Asset Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Asset Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/asset-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Assets Page ---", flush=True)
    page.goto(f"{BASE}/asset-hub/private/assets", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_assets.png"), full_page=True)
    ...
    print("\n--- Licenses Page ---", flush=True)
    page.goto(f"{BASE}/asset-hub/private/licenses", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_licenses.png"), full_page=True)
    ...
    print("\n--- Capacity Page ---", flush=True)
    page.goto(f"{BASE}/asset-hub/private/capacity", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_capacity.png"), full_page=True)
