...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/ip-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/ip-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows IP Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("IP Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/ip-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Ip-Addresses Page ---", flush=True)
    page.goto(f"{BASE}/ip-hub/private/ip-addresses", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_ip-addresses.png"), full_page=True)
    ...
    print("\n--- Dns-Records Page ---", flush=True)
    page.goto(f"{BASE}/ip-hub/private/dns-records", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_dns-records.png"), full_page=True)
    ...
    print("\n--- Dhcp-Scopes Page ---", flush=True)
    page.goto(f"{BASE}/ip-hub/private/dhcp-scopes", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_dhcp-scopes.png"), full_page=True)

    run_test(
        "Dhcp-Scopes: page loads",
        lambda: expect(page.locator("h2, h3").first).to_be_visible(),
    )
