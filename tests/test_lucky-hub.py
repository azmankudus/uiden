...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/lucky-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/lucky-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Lucky Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Lucky Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/lucky-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Events Page ---", flush=True)
    page.goto(f"{BASE}/lucky-hub/private/events", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_events.png"), full_page=True)
    ...
    print("\n--- Participants Page ---", flush=True)
    page.goto(f"{BASE}/lucky-hub/private/participants", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_participants.png"), full_page=True)
    ...
    print("\n--- Results Page ---", flush=True)
    page.goto(f"{BASE}/lucky-hub/private/results", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_results.png"), full_page=True)
