...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/software-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/software-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Software Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Software Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/software-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Inventory Page ---", flush=True)
    page.goto(f"{BASE}/software-hub/private/inventory", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_inventory.png"), full_page=True)
    ...
    print("\n--- Repositories Page ---", flush=True)
    page.goto(f"{BASE}/software-hub/private/repositories", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_repositories.png"), full_page=True)
    ...
    print("\n--- Updates Page ---", flush=True)
    page.goto(f"{BASE}/software-hub/private/updates", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_updates.png"), full_page=True)
