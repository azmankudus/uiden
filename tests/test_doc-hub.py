...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/doc-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/doc-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Doc Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Doc Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/doc-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    page.goto(f"{BASE}/doc-hub/private/templates", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_templates.png"), full_page=True)
    ...
    print("\n--- Documents Page ---", flush=True)
    page.goto(f"{BASE}/doc-hub/private/documents", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_documents.png"), full_page=True)
    ...
    print("\n--- Generator Page ---", flush=True)
    page.goto(f"{BASE}/doc-hub/private/generator", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_generator.png"), full_page=True)
