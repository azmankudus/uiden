...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/user-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/user-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows User Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("User Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/user-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Users Page ---", flush=True)
    page.goto(f"{BASE}/user-hub/private/users", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_users.png"), full_page=True)
    ...
    print("\n--- Groups Page ---", flush=True)
    page.goto(f"{BASE}/user-hub/private/groups", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_groups.png"), full_page=True)
    ...
    page.goto(f"{BASE}/user-hub/private/policies", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
