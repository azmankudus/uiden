...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/log-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/log-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Log Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Log Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/log-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Logs Page ---", flush=True)
    page.goto(f"{BASE}/log-hub/private/logs", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_logs.png"), full_page=True)
    ...
    print("\n--- Searches Page ---", flush=True)
    page.goto(f"{BASE}/log-hub/private/searches", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_searches.png"), full_page=True)
    ...
    page.goto(f"{BASE}/log-hub/private/alerts", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_alerts.png"), full_page=True)
