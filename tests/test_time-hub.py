...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/time-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/time-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Time Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Time Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/time-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- World-Clock Page ---", flush=True)
    page.goto(f"{BASE}/time-hub/private/world-clock", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_world-clock.png"), full_page=True)

    run_test(
        "World-Clock: page loads",
        lambda: expect(page.locator("h2, h3").first).to_be_visible(),
    )

    print("\n--- Stopwatch Page ---", flush=True)
    page.goto(f"{BASE}/time-hub/private/stopwatch", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_stopwatch.png"), full_page=True)

    run_test(
        "Stopwatch: page loads",
        lambda: expect(page.locator("h2, h3").first).to_be_visible(),
    )

    print("\n--- Timer Page ---", flush=True)
    page.goto(f"{BASE}/time-hub/private/timer", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_timer.png"), full_page=True)
