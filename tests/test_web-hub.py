...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/web-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/web-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Web Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Web Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/web-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Servers Page ---", flush=True)
    page.goto(f"{BASE}/web-hub/private/servers", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_servers.png"), full_page=True)
    ...
    print("\n--- Virtual-Hosts Page ---", flush=True)
    page.goto(f"{BASE}/web-hub/private/virtual-hosts", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_virtual-hosts.png"), full_page=True)

    run_test(
        "Virtual-Hosts: page loads",
        lambda: expect(page.locator("h2, h3").first).to_be_visible(),
    )

    print("\n--- Ssl-Status Page ---", flush=True)
    page.goto(f"{BASE}/web-hub/private/ssl-status", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_ssl-status.png"), full_page=True)

    run_test(
        "Ssl-Status: page loads",
        lambda: expect(page.locator("h2, h3").first).to_be_visible(),
    )
