...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/mark-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/mark-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Mark Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Mark Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/mark-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Files Page ---", flush=True)
    page.goto(f"{BASE}/mark-hub/private/files", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_files.png"), full_page=True)
    ...
    print("\n--- Editor Page ---", flush=True)
    page.goto(f"{BASE}/mark-hub/private/editor", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_editor.png"), full_page=True)
    ...
    print("\n--- Preview Page ---", flush=True)
    page.goto(f"{BASE}/mark-hub/private/preview", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_preview.png"), full_page=True)
