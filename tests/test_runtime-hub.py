...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/runtime-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/runtime-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Runtime Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Runtime Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/runtime-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Environments Page ---", flush=True)
    page.goto(f"{BASE}/runtime-hub/private/environments", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_environments.png"), full_page=True)
    ...
    print("\n--- Deployments Page ---", flush=True)
    page.goto(f"{BASE}/runtime-hub/private/deployments", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_deployments.png"), full_page=True)
    ...
    print("\n--- Configs Page ---", flush=True)
    page.goto(f"{BASE}/runtime-hub/private/configs", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_configs.png"), full_page=True)
