...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/auto-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/auto-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Auto Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Auto Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/auto-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Jobs Page ---", flush=True)
    page.goto(f"{BASE}/auto-hub/private/jobs", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_jobs.png"), full_page=True)
    ...
    print("\n--- Scripts Page ---", flush=True)
    page.goto(f"{BASE}/auto-hub/private/scripts", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_scripts.png"), full_page=True)
    ...
    print("\n--- Schedules Page ---", flush=True)
    page.goto(f"{BASE}/auto-hub/private/schedules", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_schedules.png"), full_page=True)
