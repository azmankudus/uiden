...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/secret-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/secret-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Secret Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Secret Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/secret-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Secrets Page ---", flush=True)
    page.goto(f"{BASE}/secret-hub/private/secrets", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_secrets.png"), full_page=True)
    ...
    print("\n--- Keys Page ---", flush=True)
    page.goto(f"{BASE}/secret-hub/private/keys", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_keys.png"), full_page=True)
    ...
    page.goto(f"{BASE}/secret-hub/private/policies", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
