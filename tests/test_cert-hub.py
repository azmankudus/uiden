...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/cert-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/cert-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Cert Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Cert Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/cert-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Certificates Page ---", flush=True)
    page.goto(f"{BASE}/cert-hub/private/certificates", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_certificates.png"), full_page=True)
    ...
    print("\n--- Key-Store Page ---", flush=True)
    page.goto(f"{BASE}/cert-hub/private/key-store", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_key-store.png"), full_page=True)

    run_test(
        "Key-Store: page loads",
        lambda: expect(page.locator("h2, h3").first).to_be_visible(),
    )

    print("\n--- Csr-Generator Page ---", flush=True)
    page.goto(f"{BASE}/cert-hub/private/csr-generator", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_csr-generator.png"), full_page=True)

    run_test(
        "Csr-Generator: page loads",
        lambda: expect(page.locator("h2, h3").first).to_be_visible(),
    )
