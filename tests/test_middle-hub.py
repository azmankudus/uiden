...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/middle-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/middle-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Middle Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Middle Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/middle-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    page.goto(f"{BASE}/middle-hub/private/servers", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    ...
    page.goto(f"{BASE}/middle-hub/private/deployments", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    ...
    print("\n--- Configurations Page ---", flush=True)
    page.goto(f"{BASE}/middle-hub/private/configurations", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_configurations.png"), full_page=True)
