...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/metrics-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/metrics-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Metrics Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Metrics Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/metrics-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Monitors Page ---", flush=True)
    page.goto(f"{BASE}/metrics-hub/private/monitors", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_monitors.png"), full_page=True)
    ...
    print("\n--- Alerts Page ---", flush=True)
    page.goto(f"{BASE}/metrics-hub/private/alerts", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_alerts.png"), full_page=True)
    ...
    print("\n--- Dashboards Page ---", flush=True)
    page.goto(f"{BASE}/metrics-hub/private/dashboards", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_dashboards.png"), full_page=True)
