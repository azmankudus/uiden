...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/patch-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/patch-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Patch Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Patch Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/patch-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Vulnerabilities Page ---", flush=True)
    page.goto(f"{BASE}/patch-hub/private/vulnerabilities", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_vulnerabilities.png"), full_page=True)
    ...
    print("\n--- Patches Page ---", flush=True)
    page.goto(f"{BASE}/patch-hub/private/patches", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_patches.png"), full_page=True)
    ...
    page.goto(f"{BASE}/patch-hub/private/deployments", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_deployments.png"), full_page=True)
