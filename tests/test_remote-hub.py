...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/remote-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/remote-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Remote Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Remote Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/remote-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Sessions Page ---", flush=True)
    page.goto(f"{BASE}/remote-hub/private/sessions", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_sessions.png"), full_page=True)
    ...
    print("\n--- Machines Page ---", flush=True)
    page.goto(f"{BASE}/remote-hub/private/machines", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_machines.png"), full_page=True)
    ...
    print("\n--- Policies Page ---", flush=True)
    page.goto(f"{BASE}/remote-hub/private/policies", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_policies.png"), full_page=True)
