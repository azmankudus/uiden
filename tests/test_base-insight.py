...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/base-insight")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/base-insight/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Base Insight branding",
        lambda: expect(page.locator("h1")).to_contain_text("Base Insight"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/base-insight/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    page.goto(f"{BASE}/base-insight/private/scans", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    ...
    print("\n--- Benchmarks Page ---", flush=True)
    page.goto(f"{BASE}/base-insight/private/benchmarks", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_benchmarks.png"), full_page=True)
    ...
    page.goto(f"{BASE}/base-insight/private/reports", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
