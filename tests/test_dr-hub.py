...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/dr-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/dr-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows DR Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("DR Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/dr-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Plans Page ---", flush=True)
    page.goto(f"{BASE}/dr-hub/private/plans", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_plans.png"), full_page=True)
    ...
    print("\n--- Runbooks Page ---", flush=True)
    page.goto(f"{BASE}/dr-hub/private/runbooks", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_runbooks.png"), full_page=True)
    ...
    print("\n--- Tests Page ---", flush=True)
    page.goto(f"{BASE}/dr-hub/private/tests", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_tests.png"), full_page=True)
