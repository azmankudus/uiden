...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/event-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/event-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Event Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Event Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/event-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    page.goto(f"{BASE}/event-hub/private/events", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    ...
    print("\n--- Calendar Page ---", flush=True)
    page.goto(f"{BASE}/event-hub/private/calendar", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_calendar.png"), full_page=True)
    ...
    print("\n--- Reminders Page ---", flush=True)
    page.goto(f"{BASE}/event-hub/private/reminders", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_reminders.png"), full_page=True)
