...
SNAP_DIR = Path("/tmp/uiden-screenshots/apps/keep-hub")
...
with sync_playwright() as p:
    ...
    page.goto(f"{BASE}/keep-hub/public", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "01_public_landing.png"), full_page=True)

    run_test(
        "Public Landing: shows Keep Hub branding",
        lambda: expect(page.locator("h1")).to_contain_text("Keep Hub"),
    )
    run_test(
    ...
    page.goto(f"{BASE}/keep-hub/private", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.wait_for_timeout(2000)
    ...
    # Feature pages
    # ============================================================
    print("\n--- Backups Page ---", flush=True)
    page.goto(f"{BASE}/keep-hub/private/backups", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "03_backups.png"), full_page=True)
    ...
    page.goto(f"{BASE}/keep-hub/private/policies", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "04_policies.png"), full_page=True)
    ...
    print("\n--- Recovery Page ---", flush=True)
    page.goto(f"{BASE}/keep-hub/private/recovery", timeout=15000)
    page.wait_for_load_state("networkidle", timeout=15000)
    page.screenshot(path=str(SNAP_DIR / "05_recovery.png"), full_page=True)
