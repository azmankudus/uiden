    ...
    <li>
      <Show when={hasChildren()} fallback={
        ...
        <button type="button" onClick={() => setExpanded((v) => !v)}
          class="w-full flex items-center gap-2.5 px-3 py-1.5 rounded-lg text-sm text-text-secondary hover:text-text-primary hover:bg-surface-2 transition-colors"
          style={{ "padding-left": `${12 + props.depth * 16}px` }}>
          <AppIcon icon={props.item.icon} size={16} class="flex-shrink-0" />
...
export default function SideNav(props: { items: NavItem[] }) {
  ...
  return (
    <>
      <aside class="fixed left-0 top-[53px] bottom-0 z-40 flex flex-col bg-surface-1/80 backdrop-blur-lg border-r border-surface-3/40 transition-all duration-300"
      ...
      <div class="transition-all duration-300" style={{ "margin-left": collapsed() ? "60px" : "260px", "padding-top": "53px" }}>
        {props.children}
      </div>
    </>
  );
