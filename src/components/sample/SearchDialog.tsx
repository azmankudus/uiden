...
import { PUBLIC_SEARCH, PRIVATE_SEARCH, type SearchItem } from "~/lib/sample/navigation";
...
export default function SearchDialog() {
  ...
  const items = createMemo(() => {
    const all: SearchItem[] = auth.isLoggedIn()
      ? [...PUBLIC_SEARCH, ...PRIVATE_SEARCH]
      : PUBLIC_SEARCH;
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-surface-1/60 border border-surface-3/50 text-text-muted hover:text-text-secondary hover:border-surface-3 text-sm">
        <AppIcon icon="lucide:search" size={15} />
        <span class="hidden sm:inline">Search...</span>
        <kbd class="hidden sm:inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-surface-2 text-[10px] font-body text-text-muted border border-surface-3">
