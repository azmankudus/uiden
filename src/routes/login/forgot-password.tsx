      ...
      >
        <div class="text-center mb-8">
          ...
          <h1 class="font-display text-2xl font-bold tracking-tight text-text-primary">
            {submitted() ? t().checkEmail : t().resetPassword}
          </h1>
          <p class="text-text-secondary mt-2 text-sm">
            {submitted() ? t().resetLinkSent : t().resetDesc}
          </p>
        </div>
        ...
        <div class="bg-surface-1/80 backdrop-blur-sm border border-surface-3/60 rounded-2xl p-6 sm:p-8 shadow-xl">
          ...
          >
            <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} class="space-y-5">
              <div>
                <label class="block text-sm font-medium text-text-secondary mb-1.5">{t().emailAddress}</label>
                <div class="relative">
        ...
        <p class="text-center mt-6 text-sm text-text-muted">
          <A href="/login" class="inline-flex items-center gap-1 text-brand font-medium hover:underline underline-offset-2">
