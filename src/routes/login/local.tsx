...
export default function LocalLogin() {
  ...
  onMount(() => {
    if (auth.isLoggedIn()) {
      navigate(searchParams.redirect || ROUTES.apps, { replace: true });
      return;
    }
      ...
      >
        <div class="text-center mb-8">
          <div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-brand-dim border border-brand/20 mb-4 animate-float">
            <AppIcon icon="lucide:key-round" size={32} style={{ color: "var(--color-brand)" }} />
          </div>
          <h1 class="font-display text-4xl font-extrabold tracking-tight">
            <span style={{ color: "var(--color-brand)" }}>{t().localTitle}</span>{" "}
            <span class="text-text-primary">{t().localSubtitle}</span>
          </h1>
          <p class="text-text-secondary mt-2">{t().localDesc}</p>
        </div>
        ...
        <div class="bg-surface-1/80 backdrop-blur-sm border border-surface-3/60 rounded-2xl p-6 sm:p-8 shadow-xl">
          <form onSubmit={handleSubmit} class="space-y-5">
            ...
            <div>
              <label class="block text-sm font-medium text-text-secondary mb-1.5">{t().emailOrUsername}</label>
              <div class="relative">
            ...
            <div class="flex items-center justify-between">
              <label class="flex items-center gap-2 cursor-pointer">
                ...
                <span class="text-sm text-text-secondary">{t().rememberMe}</span>
              </label>
              <A href="/login/forgot-password" class="text-sm text-brand hover:underline underline-offset-2">
                {t().forgotPassword}
              </A>
            </div>
          ...
          <div class="relative my-6">
            ...
            <div class="relative flex justify-center text-xs">
              <span class="bg-surface-1 px-3 text-text-muted uppercase tracking-wider">{t().orOtherMethods}</span>
            </div>
          </div>

          <div class="grid grid-cols-2 gap-3">
            <button
              onClick={() => navigate("/login/ad-ldap")}
        ...
        <p class="text-center mt-6 text-sm text-text-muted">
          {t().pageLocal} •{" "}
          <A href="/login/register" class="text-brand font-medium hover:underline underline-offset-2">
            {t().noAccount}
          </A>
        </p>
      </div>
