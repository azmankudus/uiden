  ...
  const sections: NavSection[] = [
    {
      title: "Navigation",
      items: [
        { label: "Dashboard", icon: "lucide:layout-dashboard", path: ROUTES.apps },
      ],
    },
    {
      title: app()?.name || "App",
      items: [
        { label: "Overview", icon: app()?.icon || "lucide:box", path: `/${params.app}/private` },
        { label: "Public Page", icon: "lucide:globe", path: `/${params.app}/public` },
      ],
    },
  ];
        ...
        <PrivateLayout name={a().name} icon={a().icon} slug={a().slug} sections={sections}>
          <div classList={{ "animate-fade-up": mounted(), "opacity-0": !mounted() }}>
            <div class="p-6 rounded-2xl bg-surface-1 border border-surface-3/30 text-center">
              ...
              <p class="mt-4 text-text-secondary text-sm">This is the private dashboard for {a().name}.</p>
              <p class="text-text-muted text-xs mt-1">Authenticated features and content will appear here.</p>
            </div>
          </div>
        </PrivateLayout>
      )}
