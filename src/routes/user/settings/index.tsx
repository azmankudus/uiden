...
export default function UserSettingsPage() {
  ...
  const tabs = [
    { id: "account", label: "Account", icon: "lucide:circle-user" },
    { id: "profile", label: "Profile", icon: "lucide:user" },
    { id: "personalization", label: "Personalization", icon: "lucide:palette" },
  ...
  return (
    <Show when={mounted()}>
      <PrivateLayout name={auth.user()?.displayName || "User"} icon="lucide:circle-user" slug="ayam-goreng" nav={settingsNav} searchItems={settingsSearch}>
        <div class={p.contentWidth() === "wide" ? "page-enter" : "max-w-4xl mx-auto page-enter"}>
          <div class="mb-8">
            ...
            <div class="flex items-center gap-4">
              <div class="relative group">
                <div class="w-16 h-16 rounded-2xl bg-brand-dim border border-brand/20 flex items-center justify-center overflow-hidden">
                  <AppIcon icon="lucide:circle-user" size={32} style={{ color: "var(--color-brand)" }} />
                </div>
                <button type="button" class="absolute inset-0 rounded-2xl bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
                  <AppIcon icon="lucide:camera" size={20} style={{ color: "white" }} />
                </button>
              </div>
              <div>
                <h1 class="font-display text-2xl font-bold text-text-primary">User Settings</h1>
                <p class="text-sm text-text-secondary">{auth.user()?.role} · {auth.user()?.appCount} apps · @{auth.user()?.username}</p>
              </div>
            </div>
          </div>
                ...
                <For each={tabs}>
                  {(tab) => (
                    <button type="button" onClick={() => setActiveTab(tab.id)}
                      ...
                      classList={{ "bg-brand-dim text-brand font-medium": activeTab() === tab.id, "text-text-secondary hover:text-text-primary hover:bg-surface-2": activeTab() !== tab.id }}>
                ...
                <For each={tabs}>
                  {(tab) => (
                    <button type="button" onClick={() => setActiveTab(tab.id)}
                      ...
                      classList={{ "bg-brand-dim text-brand font-medium": activeTab() === tab.id, "text-text-secondary bg-surface-2": activeTab() !== tab.id }}>
                      {tab.label}
                    </button>
                  )}
                </For>
