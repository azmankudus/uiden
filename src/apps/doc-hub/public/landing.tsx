...
const features = [
  { icon: "lucide:file-text", title: "Office/PDF/Image", desc: "Generate documents in multiple formats including DOCX, PDF, XLSX, and PNG." },
  { icon: "lucide:code", title: "Template Engine", desc: "Create reusable templates with variable substitution and conditional logic." },
  { icon: "lucide:layers", title: "Batch Processing", desc: "Process hundreds of documents simultaneously with queue management." },
  { icon: "lucide:pencil", title: "Custom Formatting", desc: "Apply custom styling, headers, footers, and branding to generated documents." },
];

const stats = [
  { value: "48", label: "Templates", icon: "lucide:file-text" },
  { value: "2,847", label: "Documents", icon: "lucide:file-code" },
  { value: "67", label: "Today", icon: "lucide:sparkles" },
  { value: "2.1s", label: "Avg Time", icon: "lucide:zap" },
];

export default function DocHubLanding() {
    ...
    <div class="pt-16 min-h-screen bg-surface-0">
      <section class="max-w-5xl mx-auto px-6 pt-24 pb-20 text-center" classList={{ "animate-fade-up": mounted(), "opacity-0": !mounted() }}>
        ...
        <h1 class="font-display text-4xl md:text-6xl font-bold tracking-tight text-text-primary leading-tight mb-6">
          Document <span class="text-brand">Generation</span><br />Engine
        </h1>
        <p class="text-lg text-text-secondary max-w-2xl mx-auto mb-10">
          Automate document creation with templates, batch processing, and multi-format output.
        </p>
        <div class="flex items-center justify-center gap-4 flex-wrap">
          <A href="/doc-hub/private" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold bg-brand text-surface-0 hover:brightness-110">
            <AppIcon icon="lucide:layout-dashboard" size={18} />
            Dashboard
          </A>
          <A href="/doc-hub/private/generator" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold bg-surface-2 text-text-primary hover:bg-surface-3">
      ...
      <footer class="border-t border-surface-3/30">
        <div class="max-w-5xl mx-auto px-6 py-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <span class="text-sm text-text-muted">&copy; 2026 Doc-Hub</span>
        </div>
      </footer>
    </div>
  );
