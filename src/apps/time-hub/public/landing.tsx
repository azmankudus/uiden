...
const features = [
  { icon: "lucide:globe", title: "World Clock", desc: "Track time across multiple time zones with a beautiful, easy-to-read display." },
  { icon: "lucide:timer", title: "Stopwatch", desc: "Precise stopwatch with lap timing, split tracking, and export capabilities." },
  { icon: "lucide:clock", title: "Timer", desc: "Countdown timers with presets, custom durations, and notification alerts." },
  { icon: "lucide:bell", title: "Alarms", desc: "Set and manage alarms with customizable sounds and recurring schedules." },
];

const stats = [
  { value: "8", label: "World Clocks", icon: "lucide:globe" },
  { value: "47", label: "Stopwatches Used", icon: "lucide:timer" },
  { value: "3", label: "Active Timers", icon: "lucide:clock" },
  { value: "12", label: "Alarms Set", icon: "lucide:bell" },
];

export default function TimeHubLanding() {
    ...
    <div class="pt-16 min-h-screen bg-surface-0">
      <section class="max-w-5xl mx-auto px-6 pt-24 pb-20 text-center" classList={{ "animate-fade-up": mounted(), "opacity-0": !mounted() }}>
        ...
        <h1 class="font-display text-4xl md:text-6xl font-bold tracking-tight text-text-primary leading-tight mb-6">
          Time <span class="text-brand">Management</span><br />Tools
        </h1>
        <p class="text-lg text-text-secondary max-w-2xl mx-auto mb-10">
          World clocks, stopwatches, timers, and alarms — all the time tools you need in one place.
        </p>
        <div class="flex items-center justify-center gap-4 flex-wrap">
          <A href="/time-hub/private" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold bg-brand text-surface-0 hover:brightness-110">
            <AppIcon icon="lucide:layout-dashboard" size={18} />
            Dashboard
          </A>
          <A href="/time-hub/private/world-clock" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold bg-surface-2 text-text-primary hover:bg-surface-3">
      ...
      <footer class="border-t border-surface-3/30">
        <div class="max-w-5xl mx-auto px-6 py-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <span class="text-sm text-text-muted">&copy; 2026 Time-Hub</span>
        </div>
      </footer>
    </div>
  );
