...
const features = [
  { icon: "lucide:terminal", title: "RDP/SSH/VNC", desc: "Connect to any machine using multiple protocols with a unified client experience." },
  { icon: "lucide:video", title: "Session Recording", desc: "Record all remote sessions for compliance, auditing, and security review." },
  { icon: "lucide:server", title: "Jump Server", desc: "Route connections through hardened jump servers for secure access to internal networks." },
  { icon: "lucide:shield", title: "Access Policies", desc: "Define granular policies controlling who can access what, when, and how." },
];

const stats = [
  { value: "156", label: "Machines", icon: "lucide:server" },
  { value: "23", label: "Active Sessions", icon: "lucide:monitor" },
  { value: "45ms", label: "Avg Latency", icon: "lucide:zap" },
  { value: "1.2K", label: "Recordings", icon: "lucide:video" },
];

export default function RemoteHubLanding() {
    ...
    <div class="pt-16 min-h-screen bg-surface-0">
      <section class="max-w-5xl mx-auto px-6 pt-24 pb-20 text-center" classList={{ "animate-fade-up": mounted(), "opacity-0": !mounted() }}>
        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-brand-dim text-brand text-xs font-medium mb-6">
          <AppIcon icon="lucide:monitor" size={14} />
          Remote Access Management
        </div>
        <h1 class="font-display text-4xl md:text-6xl font-bold tracking-tight text-text-primary leading-tight mb-6">
          Secure <span class="text-brand">Remote Access</span>
        </h1>
        <p class="text-lg text-text-secondary max-w-2xl mx-auto mb-10">
          Manage remote sessions, control access policies, and record all activity for complete oversight.
        </p>
        <div class="flex items-center justify-center gap-4 flex-wrap">
          <A href="/remote-hub/private" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold bg-brand text-surface-0 hover:brightness-110">
            <AppIcon icon="lucide:layout-dashboard" size={18} />
            Dashboard
          </A>
          <A href="/remote-hub/private/sessions" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold bg-surface-2 text-text-primary hover:bg-surface-3">
      ...
      <footer class="border-t border-surface-3/30">
        <div class="max-w-5xl mx-auto px-6 py-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <span class="text-sm text-text-muted">&copy; 2026 Remote-Hub</span>
        </div>
      </footer>
    </div>
  );
